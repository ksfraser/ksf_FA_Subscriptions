...
class hooks_ksf_FA_Subscriptions extends hooks {
    ...
    function install_access() {
        $security_sections[SS_SUBSCRIPTIONS] = _("Subscriptions");
        $security_areas['SA_SUBSCRIPTIONS_VIEW'] = array(
            SS_SUBSCRIPTIONS | 1, 
            _("View ")
        );
        $security_areas['SA_SUBSCRIPTIONS_MANAGE'] = array(
            SS_SUBSCRIPTIONS | 2, 
            _("Manage ")
        );
    ...
    private function ensure_composer_dependencies() {
        ...
        chdir($prev_dir);
        if ($return_code !== 0) {
            error_log('ksf_FA_Subscriptions: composer install failed: ' . implode("\n", $output));
        }
    }
}
